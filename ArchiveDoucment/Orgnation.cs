...
{
    public partial class Orgnation : Form
    {
        ...
        private void Orgnation_Load(object sender, EventArgs e)
        {
            this.BackColor = Properties.Settings.Default.color;

            try
            {
                dataGridView1.DataSource = dbsql.GetAllOrganization();
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            if (textBox1.Text.Length > 0)
            {
                try
                {
                    ...
                    {
                        dbsql.AddNewOrganization(textBox1.Text);
                        Refersh1();
                        textBox1.Focus();
                    }
                }
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (textBox2.Text.Length > 0)
            {
                ...
                {
                    name = textBox2.Text;
                    dbsql.UpdateOrganization(id, name);
                    Refersh1();
                }
                catch (Exception ex)
                { MessageBox.Show(ex.Message); }
